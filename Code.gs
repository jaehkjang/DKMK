 ...
 *  - 추천 페어링(어울리는 음식)
 * 잔 두 가지와 추천 페어링에는 5점 만점 별점이 붙는다(완벽할 때만 5점).
...
function suggestWineInfo(token, rowIndex) {
  ...
  var prompt = '너는 소믈리에다. 아래 와인 정보를 보고 서빙 방법, 어울리는 잔, 어울리는 음식, 와인 배경, 가격을 추천해라.\n' +
    ...
    '"추천페어링"에는 이 와인에 가장 잘 어울리는 음식을 2~4가지 골라 한국어로 적어라(예: "삼겹살, 훈제오리, 숙성 치즈"). ' +
    '특수 재료나 한국에서 구하기 힘든 음식보다는, 한국에서 흔히 사 먹거나 만들어 먹을 수 있는 음식 위주로 추천해라. ' +
    '"배경"에는 이 와인/생산자에 대해 알려진 이야기(스타일, 유명한 이유, 등급, 자매 와인 등)를 한국어로 두세 문장 적어라. ' +
    '아는 게 전혀 없으면 빈 문자열로 남겨라. ' +
    '"가격"에는 한국 시장 기준 대략적인 소매가를 원화 숫자로 추정해라(예: 80000). 전혀 짐작이 안 가면 0으로 남겨라. ' +
    '잔 두 개(완벽한잔, 내잔추천)와 추천페어링 모두 5점 만점 별점을 매겨라 — ' +
    '정말 완벽하게 어울릴 때만 5점을 주고, 자신 없으면 4점 이하로 줘라.\n\n' +
    '아래 JSON으로만 답해라.\n' +
    '{"품종":"...", "생산지_국가":"...", ' +
    '"서빙온도":"예: 16-18°C", "에어링시간":"예: 디캔팅 30분 또는 필요 없음", ' +
    '"완벽한잔":{"이름":"...","별점":1~5}, "내잔추천":{"이름":"...","별점":1~5} 또는 null, ' +
    '"추천페어링":{"내용":"...","별점":1~5}, "배경":"...", "가격":숫자}';

  var result = callGemini_([{ text: prompt }]);
  if (!result) throw new Error('추천을 만들지 못했어요');

  var priceGuess = parseInt(result['가격'], 10);
  var updates = {
    '품종': result['품종'] || '',
    ...
    '추천 페어링': (result['추천페어링'] && result['추천페어링']['내용']) || '',
    '추천페어링별점': (result['추천페어링'] && result['추천페어링']['별점']) || '',
    '와인배경': result['배경'] || '',
    // AI 추정치라는 걸 분명히 하려고 값 자체에 "(추정)"을 붙여둔다 — 실제로 지불한 가격과
    // 혼동되지 않게. 사용자가 실제 가격을 알면 수정 화면에서 언제든 덮어쓸 수 있다.
    '평균가격(국내·원)': (priceGuess > 0) ? (priceGuess.toLocaleString('ko-KR') + '원 (추정)') : ''
  };
