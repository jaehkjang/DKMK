...
var NEW_COLUMNS = ['어울리는잔', '서빙방법', '와인배경', '평점', '한줄평', '함께한음식', '라벨사진', '소유자',
  ...
  '베스트페어링', '베스트페어링별점', '정보갱신일', '당도', '산도', '타닌', '바디감', '구매처', '구매가격'];
 ...
 * '평점'만 예외 — 마시기 기록 자체는 markDrunk가 관리하지만, 별점을 잘못
 * 눌렀을 때 다시 마시지 않고도 고칠 수 있어야 해서 여기서 허용한다. */
 ...
 *    추천 페어링(한국에서 흔히 배달·구매하는 음식 위주 — 정통 페어링이 아니어도
 *    그럭저럭 어울리면 추천하되, 별점은 편의성이 아니라 실제 어울림 정도로만 매김)
 *  - 가격대(가격최소~가격최대 범위로 추정, 확실하면 최소=최대)
 *  - 와인 스타일 네 가지(당도·산도·타닌·바디감, 각 1~5) — 상세 화면의 스타일 차트용
...
function suggestWineInfo(token, rowIndex) {
  ...
  var prompt = '너는 소믈리에다. 아래 와인 정보를 보고 서빙 방법, 어울리는 잔, 어울리는 음식, 와인 배경, 가격대를 추천해라.\n' +
    ...
    '정말 완벽하게 어울릴 때만 5점을 주고, 자신 없으면 4점 이하로 줘라.\n\n' +
    '"당도"/"산도"/"타닌"/"바디감"에는 이 와인의 맛 스타일을 1~5 숫자로 매겨라(잘 모르는 와인이어도 종류·품종· ' +
    '생산지로 미루어 그 스타일의 일반적인 특성을 추정해라 — 절대 빈 값으로 남기지 마라). ' +
    '"당도"는 1=아주 드라이 ~ 5=아주 달콤, "산도"는 1=밋밋함 ~ 5=톡 쏘는 신맛, ' +
    '"타닌"은 1=부드러움 ~ 5=떫음(화이트·로제·스파클링처럼 타닌이 거의 없는 와인은 1~2로 낮게), ' +
    '"바디감"은 1=가벼움 ~ 5=묵직함으로 매겨라.\n\n' +
  ...
  try {
    ...
    var askText = food
      ? ('오늘 먹을 음식(여러 개면 가운데 점 · 으로 구분됨): "' + food + '"' + prefText + pairingPrinciples + '\n\n' +
        ...
        '와인을 구체적인 국가·산지·품종·와인 종류(예: 슈퍼투스칸, 샤블리, 리오하 그란 레세르바, 프로세코, 산토리니 아시르티코처럼 ' +
        '실제로 찾아서 살 수 있는 고유명사 수준) 위주로 두 문장 이내 한국어로 설명해라. ' +
        '"드라이 화이트 와인", "스파클링 와인", "산도 높은 레드"처럼 품종·산지·와인 종류 이름이 하나도 없는 ' +
        '뭉뚱그린 카테고리 설명만으로 끝내지 마라 — 그런 설명을 쓰더라도 반드시 구체적인 국가/품종/와인 종류 이름을 ' +
        '같이 못박아서 언급해라. 여러 선택지를 나열하며 얼버무리지 말고, 소믈리에로서 가장 자신 있게 ' +
        '"이거다"라고 말할 수 있는 보편적이고 정답에 가까운 조합을 확신 있게 제시해라. ' +
