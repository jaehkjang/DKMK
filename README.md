...
- `run.js` — 32개: 입장, 목록/검색, 상세, 마시기, 되돌리기, 셀러 다중 인식, 일괄 등록, 라벨 인식, 추천, 통계, 렌더링 이스케이프
- `auth_test.js` — 35개: 셀러 생성/입장, 공유(같은 이름·비번), 비번 오류, 셀러 간 데이터 분리, 남의 행 수정 차단, 무효 토큰 차단, 자동 입장
...
| `recommendByFood(token, food)` | 음식 → 보유 와인 추천 (AI, 실패 시 키워드) |
