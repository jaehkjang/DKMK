...
그래서 열쇠를 주소가 들고 다닙니다. `doGet(e)`이 `?k=` 값을 `WINE_PIN`과 대조해 맞으면 HTML의 `/*UNLOCKED*/false`를 `true`로 치환합니다. 홈 화면 바로가기는 주소가 고정이므로 계속 열린 상태가 됩니다.

PIN을 직접 입력해 통과하면 앱이 `?k=` 주소로 여는 버튼을 띄웁니다(`target="_top"`이라 바깥 창째 이동). 그 화면에서 홈 화면에 추가하면 됩니다.
